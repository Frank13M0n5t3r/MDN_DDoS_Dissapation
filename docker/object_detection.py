...
# Create a global lock to ensure that only one thread can execute a block of code at a time.
thread_lock = threading.Lock()

def thread_safe(func):
    def wrapper(*args, **kwargs):
        # Acquire the lock, execute the function, and release the lock
        with thread_lock:
...
# Define a function called process_img that takes an image as input
def process_img(image):
    # Decode the base64 encoded image data into a numpy array
    img_data = np.fromstring(base64.b64decode(image), np.uint8)
    
    # Decode the numpy array into an image matrix using OpenCV
    img_matrix = cv2.imdecode(img_data, cv2.IMREAD_COLOR)
    
    # Convert the image from BGR color space to RGB color space
    image = cv2.cvtColor(img_matrix, cv2.COLOR_BGR2RGB)
    
    # Use the loaded model to perform object detection on the image and return the result
